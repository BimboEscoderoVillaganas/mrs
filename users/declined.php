...
<body>
    ...
    <div class="container">
        ...
        <!-- Displaying Declined Reservations -->
        <br>
        <h3>Declined Reservations</h3>
        <table id="declinedTable" class="table table-striped">  
            <thead>
                <tr>
            ...
            <tbody>
                <?php 
                $declined_sql = "SELECT * FROM reservation r 
                                INNER JOIN motors b ON b.motor_id = r.motor_id
                                INNER JOIN users t ON t.user_id = r.user_id
                                WHERE t.user_id = ? AND r.status = 'declined'";
                $declined_res = $db->getRows($declined_sql, [$tid]);

                foreach ($declined_res as $r) {
                    $r_id = $r['r_id'];
                    $img = $r['b_img'];
                    $bn = $r['b_name'];
                    $bon = $r['b_model'];
                ...
                <tr>
                    ...
                    <td class="align-img"><?php echo $bn; ?></td>                    <td class="align-img"><?php echo $bon; ?></td>
                    ...
                    <td class="align-img">
                        <button class="btn btn-danger delete-btn" data-id="<?php echo $r_id; ?>">Delete</button>
                    </td>
                </tr>
                <?php
                }//end foreach loop of declined reservations
                ?>
            </tbody>
        </table>
